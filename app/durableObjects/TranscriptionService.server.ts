...
export class TranscriptionService {
  ...
  constructor(state: DurableObjectState, env: Env) {
    ...
    this.state.blockConcurrencyWhile(async () => {
      ...
      const stored = await this.state.storage.get('sessionInfo') as Record<string, string> | null
      if (stored) {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (request.headers.get('Upgrade')?.toLowerCase() === 'websocket') {
        console.error(`TranscriptionService: Direct WebSocket connections not supported [${requestId}]`)
      ...
      if (!this.env.DEEPGRAM_SECRET) {
        console.error(`TranscriptionService: DEEPGRAM_SECRET not configured [${requestId}]`)
      ...
      if (request.method === 'PUT') {
        ...
        console.warn(`TranscriptionService: Method ${request.method} not allowed [${requestId}]`)
        return new Response(JSON.stringify({
      ...
      console.error(`TranscriptionService: Unhandled error [${requestId}]`, error)
      return new Response(JSON.stringify({
  ...
  async handleSetup(request: Request, token: string): Promise<Response> {
    ...
    this.token = token
    this.userAgent = requestData.userAgent || request.headers.get('User-Agent') || 'unknown'
    ...
    if (!this.deepgramSocket || this.deepgramSocket.readyState !== WebSocket.OPEN) {
      try {
        ...
        console.error('TranscriptionService: Failed to connect to Deepgram during setup', error)
        return new Response(JSON.stringify({
  ...
  async handleAudioData(audioData: ArrayBuffer): Promise<Response> {
    ...
    try {
      ...
      console.error('TranscriptionService: Error sending audio to Deepgram', error)
  ...
  async connectToDeepgram(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      try {
        ...
        const CONNECTION_TIMEOUT = Number(this.env.DEEPGRAM_CONNECTION_TIMEOUT) || DEFAULT_CONNECTION_TIMEOUT
        ...
        socket.addEventListener('close', (event) => {
          console.log(`TranscriptionService: Deepgram connection closed (code: ${event.code})`)
        ...
        console.error('TranscriptionService: Error creating Deepgram WebSocket', error)
        reject(error)
      }
    })
  }
  ...
  handleDeepgramMessage(data: string): void {
    try {
      ...
      // Unknown message type
        ...
        console.warn('TranscriptionService: Unrecognized Deepgram response format', result)
      }
    } catch (error) {
      console.error('TranscriptionService: Error processing Deepgram response', error)
    }
  }
  ...
  async alarm(): Promise<void> {
    ...
    if (this.lastActivityTimestamp && now - this.lastActivityTimestamp > 300000) { // 5 minutes
      this.cleanupDeepgramConnection()
    }
  }
}