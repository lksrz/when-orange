 ...
 */
export default function useTranscriptionService(options: TranscriptionOptions = {}) {
   ...
   */
  const setupAudioProcessing = useCallback((track: MediaStreamTrack) => {
    ...
    if (settings.sampleRate && settings.sampleRate < 16000) {
      console.warn('Transcription service: Sample rate is low, may affect quality', settings.sampleRate)
    }
    ...
    try {
      ...
      console.error('Transcription service: Failed to set up audio processing', error)
  ...
  const attemptReconnect = useCallback(() => {
    ...
    if (!isOnline()) {
      console.log('Transcription service: Browser reports offline, waiting for online status before reconnecting')
    ...
    if (isReconnectingRef.current) {
      console.log('Transcription service: Reconnection already in progress, skipping new attempt')
      return
    }
    ...
    if (reconnectAttemptRef.current >= maxAttempts) {
      console.log(`Transcription service: Max reconnection attempts (${maxAttempts}) reached, giving up`)
    ...
    console.log(`Transcription service: Reconnection attempt ${attemptNumber} scheduled in ${Math.round(delay)}ms`)
  ...
  const createWebSocketConnection = useCallback((apiBaseUrl: string, authToken: string) => {
    try {
      ...
      console.log(`%c Transcription service: Creating WebSocket with URL: ${wsUrl}`,
        'background: blue; color: white; font-weight: bold; padding: 4px;')
        
      const ws = new WebSocket(wsUrl)
      
      // Debug the WebSocket state after creation
      console.log(`%c Transcription service: WebSocket initial state: ${ws.readyState}`,
      ...
      const connectionTimeoutId = window.setTimeout(() => {
        if (ws.readyState !== WebSocket.OPEN) {
          console.error(`%c Transcription service: Connection timeout - readyState: ${ws.readyState}`,
      ...
      ws.addEventListener('message', (event) => {
        try {
          ...
          if (typeof data !== 'object' || !data.type || typeof data.type !== 'string') {
            console.error('Transcription service: Invalid message format from server', data)
          ...
          if (data.type === 'transcription') {
            ...
            if (typeof data.text !== 'string') {
              console.error('Transcription service: Invalid transcription data, missing text', data)
              return
            }
            ...
            if (typeof data.status !== 'string' || 
              ...
              console.error('Transcription service: Invalid status value', data)
              return
            }
            ...
            if (typeof data.message !== 'string') {
              console.error('Transcription service: Invalid error format', data)
              options.onError?.(new Error('Server error (no details provided)'))
              return
            }
            ...
            console.log('Transcription service: Pong received', data.timestamp)
          } else {
            // Unknown message type
            console.warn('Transcription service: Unknown message type from server', data.type)
          }
        } catch (error) {
          console.error('Transcription service: Failed to parse message', error)
        }
      })
      
      ws.addEventListener('error', (event) => {
        console.error('%c Transcription service: WebSocket error', 
          'background: red; color: white; font-weight: bold; padding: 4px;', event)
        clearTimeout(connectionTimeoutId)
        ...
        const isCloudflareHosted = window.location.hostname.includes('cloudflare') || 
        ...
        console.log('%c Transcription WebSocket debug info:', 
        ...
        fetch('/api/transcription')
          .then(response => {
            console.log('%c Transcription service: HTTP fetch test:', 
          ...
          .catch(error => {
            console.error('%c Transcription service: HTTP fetch test failed:', 
              'background: red; color: white; font-weight: bold; padding: 4px;', error)
          })
      ...
      ws.addEventListener('close', (event) => {
        console.log(`Transcription service: WebSocket disconnected (code: ${event.code}, reason: ${event.reason || 'none'})`)
        clearTimeout(connectionTimeoutId)
        ...
        const isNormalClosure = (
          (event.code >= 1000 && event.code <= 1999) &&
          event.code !== 1006 // 1006 is abnormal closure
        )
      ...
      console.error('Transcription service: Failed to create WebSocket', error)
  ...
  }, [options, attemptReconnect, setupPingInterval, setupAudioProcessing, stopAudioProcessing])
  ...
  const setupWebSocket = useCallback(() => {
    ...
    if (window.location.hostname.includes('cloudflare') || 
      ...
      if (localStorage.getItem('use_old_transcription_path') === 'true') {
        ...
        console.log('%c Using OLD transcription path for debugging', 
    ...
    console.log(`%c Transcription service: Checking API availability at ${apiBaseUrl}`, 
    ...
    fetch(apiBaseUrl)
      ...
      .then(data => {
        console.log('%c Transcription service: API check successful', 
          'background: green; color: white; font-weight: bold; padding: 4px;', data)
          
        // Now we can proceed with creating the WebSocket
        createWebSocketConnection(apiBaseUrl, authToken)
      })
      .catch(error => {
        console.error('%c Transcription service: API check failed', 
          'background: red; color: white; font-weight: bold; padding: 4px;', error)
        
        options.onError?.(new Error(`Failed to connect to transcription service: ${error.message}`))
  ...
  const isDeviceChanged = useCallback((track: MediaStreamTrack) => {
    ...
    if (newSettings.sampleRate !== oldSettings.sampleRate ||
  ...
  const startTranscription = useCallback((track: MediaStreamTrack) => {
    ...
    if (deviceChanged && audioTrackRef.current) {
      console.log('Transcription service: Restarting audio processing due to device change')
      stopAudioProcessing()
    }
