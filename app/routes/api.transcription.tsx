...
export const loader = async ({ request, context }: LoaderFunctionArgs) => {
	...
	if (!context.env.transcriptionService) {
		console.error(
			`🔌 Transcription API [${requestId}]: transcriptionService not configured in env`
		)
		return new Response(
			JSON.stringify({
	...
	if (request.method === 'OPTIONS') {
		console.log(
			`🔌 Transcription API [${requestId}]: Handling OPTIONS request (CORS preflight)`
		)
	...
	if (!upgradeHeader || upgradeHeader.toLowerCase() !== 'websocket') {
		console.log(
			`🔌 Transcription API [${requestId}]: Normal HTTP request received, returning service info`
		)
	...
	try {
		console.log(
			`🔌 Transcription API [${requestId}]: Processing WebSocket upgrade request`,
			{
		...
		console.log(
			`🔌 Transcription API [${requestId}]: Using DO ID: ${idString.toString()}`
		)
		...
		const pingInterval = setInterval(() => {
			if (server.readyState === WebSocket.OPEN) {
				try {
					...
					console.error(
						`🔌 Transcription API [${requestId}]: Error sending ping:`,
						error
					)
		...
		server.addEventListener('message', async (event: MessageEvent) => {
			try {
				...
				console.log(
					`🔌 Transcription API [${requestId}]: Forwarding message to DO`,
					{
				...
				if (response.ok) {
					...
					// Error handling
					console.error(
						`🔌 Transcription API [${requestId}]: DO returned error status: ${response.status}`
					)
					server.send(
						JSON.stringify({
							type: 'error',
				...
				console.error(
					`🔌 Transcription API [${requestId}]: Error forwarding message:`,
					error
				)
				server.send(
					JSON.stringify({
		...
		server.addEventListener('close', async (event: CloseEvent) => {
			try {
				console.log(
					`🔌 Transcription API [${requestId}]: WebSocket closed with code ${event.code} reason: ${event.reason || 'none'}`
				)
				...
				console.error(
					`🔌 Transcription API [${requestId}]: Error handling WebSocket close:`,
					error
				)
			}
		})
		...
		server.addEventListener('error', (event: Event) => {
			console.error(
				`🔌 Transcription API [${requestId}]: WebSocket error:`,
				event
			)
			// Clean up the ping interval
			clearInterval(pingInterval)
		...
		try {
			console.log(
				`🔌 Transcription API [${requestId}]: Initializing DO session`
			)
			...
			if (!initResponse.ok) {
				...
				try {
					...
					const errorData = (await initResponse.json()) as {
						error?: string
						message?: string
					}
					errorText =
				...
				console.error(
					`🔌 Transcription API [${requestId}]: Error initializing session:`,
					{
				...
				console.log(
					`🔌 Transcription API [${requestId}]: DO initialization successful`,
					responseData
				)
			...
			console.error(
				`🔌 Transcription API [${requestId}]: Error during initialization:`,
				error
			)
			server.send(
				JSON.stringify({
		...
		console.log(
			`🔌 Transcription API [${requestId}]: Returning WebSocket upgrade response`
		)
		return new Response(null, {
			status: 101,
			webSocket: pair[0],
		...
		console.error(
			`🔌 Transcription API [${requestId}]: Error processing WebSocket upgrade:`,
			error
		)
		return new Response(
			JSON.stringify({
