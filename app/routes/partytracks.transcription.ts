...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  invariant(env.transcriptionService, 'TranscriptionService binding is required')
  
  // Ensure we have an AUTH_SECRET for token generation
  invariant(env.AUTH_SECRET, 'AUTH_SECRET is required for secure transcription service')
  ...
  try {
    ...
    return new Response('Authentication failed or server error', { status: 500 })
  }
}