 ...
 */
export async function generateAuthToken(username: string, secret: string): Promise<string> {
  // Validate inputs
  if (!secret) {
    throw new Error('AUTH_SECRET environment variable is required for token generation')
  }
  
  if (!username || typeof username !== 'string' || username.length < 1 || username.length > 255) {
