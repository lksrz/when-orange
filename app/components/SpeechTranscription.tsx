...
const getMaxTranscriptionHistory = () => {
  ...
  if (window.navigator && 'deviceMemory' in window.navigator) {
    ...
    return memory > 4 ? DEFAULT_MAX_TRANSCRIPTION_HISTORY : LOW_MEMORY_MAX_TRANSCRIPTION_HISTORY
  }
...
export const SpeechTranscription: FC = () => {
  ...
  // Debugging info
  console.log('%c Transcription status:', 
  ...
  useEffect(() => {
    const debugFlag = new URLSearchParams(window.location.search).get('debug_transcription');
    if (debugFlag) {
      localStorage.setItem('use_old_transcription_path', debugFlag === 'old' ? 'true' : 'false');
      console.log('%c Transcription debug flag set:', 
        'background: purple; color: white; font-weight: bold;',
        debugFlag
      );
    }
  ...
  const addTranscription = useCallback((text: string, isFinal: boolean) => {
    const now = Date.now()
  ...
  }, [audioTrack, hasTranscriptionCredentials, isTranscribing, transcriptionService])
